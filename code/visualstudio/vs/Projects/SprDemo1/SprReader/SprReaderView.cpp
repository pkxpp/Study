...
BEGIN_MESSAGE_MAP(CSprReaderView, CView)
	...
	ON_COMMAND(ID_32775, &CSprReaderView::OnSynthesisSprite)
END_MESSAGE_MAP()
...
//				m_ppSprFrameList[i] = (FSprFrame*)(pSprite + m_pSprOffsList[i].Offset);
	...
	rect.left = 320 - m_pSprHead->Width/2;
	rect.top = 240 - m_pSprHead->Height/2;
...
void CSprReaderView::OnSynthesisSprite()
{
	// TODO: Add your command handler code here
	m_pKCDlgSynSpr = new KDlgSynSprite;
	m_pKCDlgSynSpr->Create(KDlgSynSprite::IDD);

	m_pKCDlgSynSpr->ShowWindow(SW_SHOW);//显示对话框
}
