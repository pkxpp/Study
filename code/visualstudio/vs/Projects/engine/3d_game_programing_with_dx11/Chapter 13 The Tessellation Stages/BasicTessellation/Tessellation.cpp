...
: D3DApp(hInstance), mQuadPatchVB(0), mVB(0), mIB(0), 
...
void BasicTessellation::DrawScene()
{
    ...
    for(UINT p = 0; p < techDesc.Passes; ++p)
    {
		...
		Effects::TessellationFX->TessTech->GetPassByIndex(p)->Apply(0, md3dImmediateContext);
		//md3dImmediateContext->Draw(mSphereIndexCount, 0);
		md3dImmediateContext->RSSetState(RenderStates::WireframeRS);
...
void BasicTessellation::BuildGeometryBuffers()
{
	GeometryGenerator::MeshData sphere;
	GeometryGenerator geoGen;

	geoGen.CreateGeosphere(0.5, 1, sphere);
