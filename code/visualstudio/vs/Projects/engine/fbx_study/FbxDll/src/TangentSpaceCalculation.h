...
private:	// -----------------------------------------------------------------
	...
	public:
		...
		CBase33() { }
		CBase33(CVec3 Uval, CVec3 Vval, CVec3 Nval) { u=Uval; v=Vval; n=Nval; texcoordNo = 0; }
...
private: // ----------------------------------------------------------------------------------------
	...
	unsigned int AddUV2Base( std::multimap<CBaseIndex,unsigned int,CBaseIndexOrder> &inMap, const unsigned int indwPosNo, const unsigned int indwTexcoordNo, const unsigned int indwNormNo, const CVec3 &inU, const CVec3 &inV, const CVec3 &inNormN );
...
//	DebugMesh(inInput);		// only for debugging - slow
	...
	// calculate the base vectors per triangle -------------------------------------------
	{
		for(unsigned int i=0;i<dwTriCount;i++)
		{
			...
			// TODO why normal inverse? (add by dzl 140923)
			vN = -vN;
	...
	// distribute the uv vectors to the vertices
	{
		...
		for(unsigned int i=0;i<dwTriCount;i++)
		{
			...
			for(e=0;e<3;e++)	
			{
				...
				Indx.p[e]=AddUV2Base(mBaseMap,dwPos[e],dwUV[e],dwNorm[e],TriBase.u*fWeight,TriBase.v*fWeight,normalize(TriBase.n));
			}
...
unsigned int CTangentSpaceCalculation<InputProxy>::AddUV2Base( std::multimap<CBaseIndex,unsigned int,CBaseIndexOrder> &inMap,
	const unsigned int indwPosNo, const unsigned int indwTexcoordNo, const unsigned int indwNormNo, const CVec3 &inU, const CVec3 &inV, const CVec3 &inNormN )
{
	// no mesh is perfect
//	assert(IsNormalized(inNormN));
	...
	for(;iFind!=iFindEnd;++iFind)
	{
		...
		{
			bool bTexcoordSameOrEmpty = true;
			if (refFound.texcoordNo != 0)
			{
				if (indwTexcoordNo != refFound.texcoordNo)
				{
					bTexcoordSameOrEmpty = false;
				}
			}

			if (!bTexcoordSameOrEmpty)
				continue;
...
//			bool bHalfAngleCheck=normalize(inU+inV) * normalize(vRotHalf) > cos(fThreshold);
	...
	if (!refBaseUV.texcoordNo)
		refBaseUV.texcoordNo = indwTexcoordNo;
