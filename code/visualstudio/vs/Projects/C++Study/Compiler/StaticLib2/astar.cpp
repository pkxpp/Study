...
// 计算G值
inline uint16_t AStar::calcul_g_value(Node *parent, const KVec2 &current)
{
    uint16_t g_value = KVec2::Distance(current, parent->pos) == 2 ? oblique_val_ : step_val_;
    return g_value += parent->g;
}

// 计算F值
inline uint16_t AStar::calcul_h_value(const KVec2 &current, const KVec2 &end)
{
    unsigned int h_value = KVec2::Distance(end, current);
    return h_value * step_val_;
}

// 节点是否存在于开启列表
inline bool AStar::in_open_list(const KVec2 &pos, Node *&out_node)
{
...
// 节点是否存在于关闭列表
inline bool AStar::in_closed_list(const KVec2 &pos)
{
...
// 是否可到达
bool AStar::can_pass(const KVec2 &pos)
{
    return (pos.x >= 0 && pos.x < width_ && pos.y >= 0 && pos.y < height_) ? can_pass_(pos) : false;
}

// 当前点是否可到达目标点
bool AStar::can_pass(const KVec2 &current, const KVec2 &destination, bool allow_corner)
{
    if (destination.x >= 0 && destination.x < width_ && destination.y >= 0 && destination.y < height_)
    {
        ...
        if (KVec2::Distance(destination, current) == 1)
        {
            return can_pass_(destination);
        }
        else if (allow_corner)
        {
            return can_pass_(destination) && (can_pass(KVec2(current.x + destination.x - current.x, current.y))
                    && can_pass(KVec2(current.x, current.y + destination.y - current.y)));
        }
    }
    return false;
}
...
// 查找附近可通过的节点
void AStar::find_can_pass_nodes(const KVec2 &current, bool corner, std::vector<KVec2> *out_lists)
{
    KVec2 destination;
    ...
    while (row_index <= max_row)
    {
        ...
        while (col_index <= max_col)
        {
            ...
            destination.x = col_index;
            destination.y = row_index;
...
// 处理未找到节点的情况
void AStar::handle_not_found_node(Node *current, Node *destination, const KVec2 &end)
{
    destination->parent = current;
...
// 执行寻路操作
std::vector<KVec2> AStar::find(const Params &param)
{
    std::vector<KVec2> paths;
...
void AStar::find(Params& param, std::vector<KVec2> vecPath)
{
    std::vector<KVec2> vecTemp = find(param);
    //vecPath.assign(vecTemp.begin(), vecTemp.end());
}
//
//void AStar::test(STest& a)
//{
//    a.a = 100;
//}
//
//void AStar::test1(Params& a)
//{
//    a.height = 1000;
//}
//
//void AStar::test2(std::vector<int> vecTest)
//{
...
//}
//
//void AStar::test3(std::vector<KVECTOR2> vecTest)
//{
//    int nSize = vecTest.size();
//    for (auto it : vecTest)
//    {
...
//}
//
//void AStar::test4(SSubTest& a)
//{
//    a.a = 100;
//}
//
//void AStar::test5(KVec2& v2Test)
//{
//    v2Test.x = 100;
//}
//
//void AStar::test6(std::vector<SSubTest> vecTest)
//{
//    int nSize = vecTest.size();
//    for (auto it : vecTest)
//    {
...
//}
//
////void AStar::test7(KVec2Test& v2Test)
////{
////    v2Test.x = 100;
////}
////
////void AStar::test8(std::vector<KVec2Test> vecTest)
////{
////    int nSize = vecTest.size();
////    for (auto it : vecTest)
////    {
...
////}
//
//void AStar::test9(std::vector<KVec2> vecTest)
//{
//    int nSize = vecTest.size();
//    for (auto it : vecTest)
//    {
