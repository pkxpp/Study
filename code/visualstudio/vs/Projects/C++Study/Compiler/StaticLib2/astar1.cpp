...
// 计算G值
inline uint16_t AStar::calcul_g_value(Node *parent, const KVectorInt2 &current)
{
...
// 计算F值
inline uint16_t AStar::calcul_h_value(const KVectorInt2 &current, const KVectorInt2 &end)
{
...
// 节点是否存在于开启列表
inline bool AStar::in_open_list(const KVectorInt2 &pos, Node *&out_node)
{
...
// 节点是否存在于关闭列表
inline bool AStar::in_closed_list(const KVectorInt2 &pos)
{
...
// 是否可到达
bool AStar::can_pass(const KVectorInt2 &pos)
{
    return (pos.x >= 0 && pos.x < width_ && pos.y >= 0 && pos.y < height_) ? can_pass_(pos) : false;
}

// 当前点是否可到达目标点
bool AStar::can_pass(const KVectorInt2 &current, const KVectorInt2 &destination, bool allow_corner)
{
    if (destination.x >= 0 && destination.x < width_ && destination.y >= 0 && destination.y < height_)
    {
        ...
        {
            return can_pass_(destination) && (can_pass(KVectorInt2(current.x + destination.x - current.x, current.y))
                    && can_pass(KVectorInt2(current.x, current.y + destination.y - current.y)));
        }
    }
    return false;
}
...
// 查找附近可通过的节点
void AStar::find_can_pass_nodes(const KVectorInt2 &current, bool corner, std::vector<KVectorInt2> *out_lists)
{
    KVectorInt2 destination;
    /*
    ...
    for (auto it : Dirs)
    {
        int nCol = current.x + it.x;
        int nRow = current.y + it.y;
        destination.Reset(nCol, nRow);
        if (can_pass(current, destination, corner))
        {
            out_lists->push_back(destination);
        }
...
// 处理未找到节点的情况
void AStar::handle_not_found_node(Node *current, Node *destination, const KVectorInt2 &end)
{
    destination->parent = current;
...
// 执行寻路操作
void AStar::find(const Params &param, std::vector<KVectorInt2>& vecOutPath)
{
    //std::vector<KVectorInt2> paths;
    ...
    // 初始化
    init(param);
    std::vector<KVectorInt2> nearby_nodes;
    ...
    while (!open_list_.empty())
    {
        ...
        if (current->pos == param.end)
        {
            ...
            {
                vecOutPath.push_back(current->pos);
                current = current->parent;
            }
            std::reverse(vecOutPath.begin(), vecOutPath.end());
            goto __end__;
        }
