...
//#include "Public/Windows/AllowWindowsPlatformAtomics.h"
...
#include "Public/Templates/UnrealTemplate.h"
...
private:
	void* GetRawItem(uint32 Index)
	{
		...
		if (!Pages[BlockIndex])
		{
			...
			if (FWindowsPlatformAtomics::InterlockedCompareExchangePointer((void**)&Pages[BlockIndex], NewBlock, nullptr) != nullptr)
			{
...
struct FIndexedPointer
{
	...
	FORCEINLINE void AtomicRead(const FIndexedPointer& Other)
	{
		...
		Ptrs = uint64(FWindowsPlatformAtomics::AtomicRead((volatile const int64*)&Other.Ptrs));
		TestCriticalStall();
	}
	...
	{
		TestCriticalStall();
		return uint64(FWindowsPlatformAtomics::InterlockedCompareExchange((volatile int64*)&Ptrs, Exchange.Ptrs, Comparand.Ptrs)) == Comparand.Ptrs;
	}
