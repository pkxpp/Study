...
{
	//ChangeMem(AllocSize);
	return KMemory::Alloc(AllocSize);
}
...
public:
	...
	LockFreeLinkAllocator_TLSCache()
	{
		...
		TlsSlot = AllocTlsSlot();
		IsValidTlsSlot(TlsSlot);
	}
...
private:
	...
	FThreadLocalCache& GetTLS()
	{
		...
		FThreadLocalCache* TLS = (FThreadLocalCache*)GetTlsValue(TlsSlot);
		if (!TLS)
		{
			TLS = new FThreadLocalCache();
			SetTlsValue(TlsSlot, TLS);
		}
		return *TLS;
	}
