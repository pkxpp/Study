...
namespace ENamedThreads
{
	...
	FORCEINLINE int GetQueueIndex(Type ThreadAndIndex)
	{
		return (ThreadAndIndex & QueueIndexMask) >> QueueIndexShift;
	}

	FORCEINLINE int GetTaskPriority(Type ThreadAndIndex)
	{
		return (ThreadAndIndex & TaskPriorityMask) >> TaskPriorityShift;
	}

	FORCEINLINE int GetThreadPriorityIndex(Type ThreadAndIndex)
	{
...
extern CORE_API int GEnablePowerSavingThreadPriorityReductionCVar;
...
public:
	 ...
	 **/
	FFunctionGraphTask(std::function<void()>&& InFunction, /*const TStatId StatId, */ENamedThreads::Type InDesiredThread)
	 ...
	 **/
	static FGraphEventRef CreateAndDispatchWhenReady(std::function<void()>&& InFunction, const/* TStatId InStatId, */const FGraphEventArray* InPrerequisites = NULL, ENamedThreads::Type InDesiredThread = ENamedThreads::AnyThread)
	{
		return TGraphTask<FFunctionGraphTask>::CreateTask(0/*InPrerequisites*/).ConstructAndDispatchWhenReady(MoveTemp(InFunction), /*InStatId,*/ InDesiredThread);
	}
	/**
	 * Create a task and dispatch it when the prerequisites are complete
	 * @param InTaskDelegate - delegate to execute when the prerequisites are complete
	 ...
	 * @return completion handle for the new task
	 **/
	static FGraphEventRef CreateAndDispatchWhenReady(std::function<void()> InFunction, /*const TStatId&& InStatId, const FGraphEventRef& InPrerequisite,*/ ENamedThreads::Type InDesiredThread = ENamedThreads::AnyThread)
	{
