...
void lprofM_enter_function(lprofP_STATE* S, char *file_defined, char *fcn_name, long linedefined, long currentline) {
  ...
  if(fcn_name != NULL) {
	...
	//  size_t nSize = sizeof(char) * (strlen("called from ") + strlen(prev_name) + 1);
    //cur_name = (char*)malloc(nSize);
	//sprintf_s(cur_name, nSize, "called from %s", prev_name);
	...
	//size_t nSize = sizeof(char) * (strlen(file_defined) + 12);
    //cur_name = (char*)malloc(nSize);
	//sprintf_s(cur_name, nSize, "%s:%li", file_defined, linedefined);
