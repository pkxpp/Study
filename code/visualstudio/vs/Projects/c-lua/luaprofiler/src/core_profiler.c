...
int lprofP_callhookOUT(lprofP_STATE* S) {
  ...
  if (strlen(name) > MAX_FUNCTION_NAME_LENGTH) {
	 ...
	 //strncpy_s(name + 1, MAX_FUNCTION_NAME_LENGTH, info->function_name, MAX_FUNCTION_NAME_LENGTH);
...
lprofP_STATE* lprofP_init_core_profiler(const char *_out_filename, int isto_printheader, float _function_call_time) {
  ...
  //randstr = tmpnam_s(NULL, 0);
  randstr = tmpnam(NULL);
  //for (s = strtok_s(randstr, "/\\", buf); s; s = strtok_s(NULL, "/\\", buf)) {
  ...
  //sprintf_s(auxs, out_filename, randstr);
  outf = fopen(auxs, "a");
  //fopen_s(outf, auxs, "a");
  if (!outf) {
    return 0;
