...
SprConverter::SprConverter(IDirect3DDevice9*	pd3dDevice)
{
...
//成功返回True，否则返回False
bool SprConverter::CreateNewSprFile(const char* pszSprName, const unsigned nFrames, const FSprHead SprHead){
	//检查文件是否存在
//	MessageBox(pszSprName, MB_OK, "ERROR");
//	AfxMessageBox(pszSprName, MB_OK, 0);
//	char temp[32];
//	itoa(nFrames, temp, 10);
//	AfxMessageBox(temp, MB_OK, 0);

	//检查文件是否存在
	if(pszSprName == NULL || nFrames == 0)
		return false;
...
//	printf("new name: %s\n", sNew.c_str());
	...
	if(SprHead.Interval != 0)
		Head.Interval = SprHead.Interval;
	else
		Head.Interval = 1;		/* 每帧间隔(默认) */

	/*
	 * for test
	CString xxx;
	xxx.Format("%d", Head.Interval);
	AfxMessageBox(xxx.GetBuffer(), MB_OK, 0);
	*/
...
//		AfxMessageBox(cszFrame, MB_OK, 0);

		D3DXIMAGE_INFO pSrcInfo;
		if( D3DXGetImageInfoFromFile(cszFrame, &pSrcInfo) != D3D_OK ) {
//			AfxMessageBox("Read image error!", MB_OK, 0);
			return false;
		}
