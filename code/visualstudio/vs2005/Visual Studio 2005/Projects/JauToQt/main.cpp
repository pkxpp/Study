...
translate_chinese(char* argv[]){
	...
	infTranslate(szDirectory, szDirectory2);
	/*
	if ( szDirectory[0] == '\0' ){
		write_log(LT_BOTH,"Error: Please specify the source root path after '-s'\n");
		return 0;
...
update_translate(char* argv[]){
	...
	infUpdate(szFileName, szFileName2, szChinese, szTranslate, nOldTrans, nClearTable, nUpdateType);
	/*
...
int infUpdate(char *szFileName, char *szFileName2, char *szChinese, char *szTranslate, int nOldTrans, int	nClearTable = 0/**/, int nUpdateType = 3)
{
    int nRet;
    if ( db_init(nClearTable) == 0 ){
        write_log(LT_BOTH,"Error: Can't init 'database operation' module!\n");
...
traverse_database(char* argv[]){
	...
	infTraverse(szChineseFileName, szTransFileName, szOperation);
	/*
