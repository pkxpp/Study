	...
	if(check(n)==1)
	{
		...
		if(n%2 == 0)
		{
			...
			for(int k = 0; k < n; k++){
				...
				bool bRet = false;
				bRet = exchange_elem(n, a[k][n-k-1]-2, 0, n-1, 0, vecIgnore);
				if (bRet)	
				{
...
vector<int> compute_col(int arrA[], int arrB[], int n, int sub, bool tag, vector<int> vecIgnore)
{
	...
	cout << "compute_col-vecIgnore.size: " << vecIgnore.size() << ", " ;
...
bool exchange_elem(int n, int nNum, int dstX, int dstY, int nTag, vector<int> vecIgnore)
{
	...
	//vecIgnore.push_back(5);
	bool bRet = false;
	bRet = equal_transfor(n, x, y, dstNum, dstX, dstY, 0, vecIgnore);
	bRet = equal_transfor(n, x, y, dstNum, dstX, dstY, 1, vecIgnore);
	return bRet;
	/*vector<int> vecElem;
